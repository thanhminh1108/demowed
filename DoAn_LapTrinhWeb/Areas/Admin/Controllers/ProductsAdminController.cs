...
namespace DoAn_LapTrinhWeb.Areas.Admin.Controllers
{
    public class ProductsAdminController : BaseController
    {
        ...
        public ActionResult Edit(Product model, ProductImages productImage)
        {
            ...
            try
            {
                if (model.ImageUploadMulti != null)
                {
                    var fileName = Path.GetFileNameWithoutExtension(model.ImageUploadMulti.fileName);
                    var extension = Path.GetExtension(model.ImageUploadMulti.fileName);
                    fileName = fileName + extension;
                    product.image = "/Content/Images/" + fileName;
                    model.ImageUploadMulti.SaveAs(Path.Combine(Server.MapPath("~/Content/Images/"), fileName));
                }
